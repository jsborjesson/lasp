...
module Lasp
  describe Params do
    context "non-variadic parameters" do
      let(:params) { described_class.new(%i[one two]) }
      ...
      it "enforces arity" do
        expect {
          params.with_args([1, 2, 3])
        }.to raise_error(Lasp::ArgumentError, "wrong number of arguments (3 for 2)")

        expect {
          params.with_args([1])
        }.to raise_error(Lasp::ArgumentError, "wrong number of arguments (1 for 2)")
      end

      it "only counts the last underscore" do
        ignored_params = described_class.new(%i[_ _ three])
